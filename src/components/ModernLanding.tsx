    ...
    <motion.button
      ...
      className={
        ...
        (variant === "primary"
          ...
          : "glass-panel text-foreground hover:bg-foreground/10 ") +
        className
      }
    >
      {variant === "primary" && (
...
function Nav() {
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center text-[13px] text-foreground/70">
        {["Product", "Capabilities", "Deploy"].map((l) => (
          <a key={l} href={`#${l.toLowerCase()}`} className="px-3 py-1.5 hover:text-foreground transition">
            {l}
          </a>
        ))}
      </div>
...
function TypewriterText({
  ...
  useEffect(() => {
    ...
    setDone(true);
    onDone?.();
  }, [started, done, charIdx, lineIdx, lines, speed, pauseBetween, onDone]);
      ...
      {lines.map((line, i) => {
        ...
        return (
          <span key={i} className={line.className}>
            {text}
            {isCurrent && !done && (
              <span className="inline-block w-[0.06em] h-[0.85em] align-middle ml-1 bg-current animate-pulse" />
            )}
            {i < lines.length - 1 && <br />}
          </span>
        );
...
// ---------- Cycling Typewriter (types a phrase, pauses, deletes, moves to next — loops forever) ----------
function CyclingTypewriter({
  ...
  useEffect(() => {
    ...
    if (mode === "typing") {
      ...
      const t = setTimeout(() => setMode("pausing"), pauseAfterType);
      return () => clearTimeout(t);
    }
  ...
  }, [mode, charIdx, phraseIdx, phrases, typeSpeed, deleteSpeed, pauseAfterType, pauseAfterDelete]);
    ...
    <section className="relative min-h-screen flex items-center justify-center pt-32 pb-20 overflow-hidden">
      ...
      <div className="max-w-6xl px-6 text-center relative">
        <motion.div
          ...
          className="inline-flex items-center gap-2 glass-panel rounded-full px-4 py-1.5 text-[12px] text-foreground/80 mb-8"
        >
        ...
        <motion.h1
          ...
          className="font-display font-semibold tracking-[-0.04em] text-[clamp(2.6rem,7vw,6.5rem)] leading-[0.95] text-foreground"
        >
          <span className="block">Deploy to Cloud</span>
          <CyclingTypewriter
            className="block holo-text"
            phrases={["Self-Healing", "Zero Downtime.", "at GenAI Speed.", "Zero Touch."]}
            typeSpeed={55}
            deleteSpeed={30}
            pauseAfterType={1200}
            pauseAfterDelete={300}
        ...
        <motion.p
          ...
          className="mt-6 max-w-2xl mx-auto text-foreground/60 text-lg leading-relaxed"
        >
        ...
        <motion.div
          ...
          className="mt-10 flex flex-wrap justify-center items-center gap-6 text-[12px] text-muted-foreground"
        >
...
function FloatingOrbs() {
  return (
    <>
      {[
        { top: "10%", left: "6%", size: 320, hue: "var(--holo-violet)", d: 0 },
        { top: "55%", left: "82%", size: 300, hue: "var(--holo-cyan)", d: 1 },
        { top: "80%", left: "20%", size: 260, hue: "var(--holo-magenta)", d: 2 },
      ].map((o, i) => (
        <motion.div
          key={i}
...
// One continuous ambient layer behind the entire page, instead of per-section
// glows — avoids any hard edges where a section's own clipping would cut a
// blob off at a border.
function PageAmbient() {
  const blobs: { top: string; left: string; size: number; hue: string }[] = [
    { top: "0%", left: "10%", size: 600, hue: "var(--holo-violet)" },
    { top: "8%", left: "78%", size: 560, hue: "var(--holo-cyan)" },
    { top: "24%", left: "35%", size: 520, hue: "var(--holo-magenta)" },
    { top: "38%", left: "85%", size: 560, hue: "var(--holo-amber)" },
    { top: "52%", left: "5%", size: 580, hue: "var(--holo-cyan)" },
    { top: "66%", left: "60%", size: 560, hue: "var(--holo-violet)" },
    { top: "80%", left: "15%", size: 540, hue: "var(--holo-magenta)" },
    { top: "94%", left: "70%", size: 560, hue: "var(--holo-amber)" },
  ];
        ...
        {capabilities.map((c, i) => (
          <TiltCard key={c.title} tint={c.tint} delay={i * 0.08}>
            <div className="flex items-start gap-4">
              ...
              <div>
                <div className="text-xs uppercase tracking-widest text-muted-foreground">{c.sub}</div>
                <h3 className="font-display text-2xl font-semibold text-foreground mt-1">{c.title}</h3>
                <p className="text-foreground/60 text-[15px] mt-3 leading-relaxed">{c.body}</p>
              </div>
            </div>
          </TiltCard>
        ))}
      ...
      <div className="max-w-6xl mx-auto mt-14 grid md:grid-cols-3 gap-5">
        {suite.map((s, i) => (
          ...
          >
            ...
            <h4 className="font-display text-xl text-foreground">{s.title}</h4>
            <p className="text-foreground/60 text-sm mt-2">{s.body}</p>
          </motion.div>
        ))}
      </div>
    </section>
  ...
  return (
    <section id="deploy" className="relative py-32 px-6">
      ...
      <div className="max-w-6xl mx-auto mt-16 relative">
        <div className="hidden md:block absolute top-24 left-[10%] right-[10%] h-px bg-gradient-to-r from-transparent via-border to-transparent" />
        <div className="grid md:grid-cols-3 gap-6 relative">
          {steps.map((s, i) => (
            ...
            >
              ...
              <div className="font-mono text-xs text-muted-foreground">STEP {s.step}</div>
              <h3 className="font-display text-2xl text-foreground mt-1">{s.title}</h3>
              <ul className="mt-6 space-y-2">
                {s.items.map((it) => (
                  <li key={it} className="flex items-center gap-2 text-sm text-foreground/70">
      ...
      <div className="max-w-6xl mx-auto mt-14 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {useCases.map((u, i) => (
          ...
          >
            ...
            <h4 className="font-display text-lg text-foreground mt-4">{u.title}</h4>
            <p className="text-foreground/55 text-sm mt-1">{u.body}</p>
          </motion.div>
        ))}
      </div>
    </section>
      ...
      <div className="max-w-6xl mx-auto mt-14 grid md:grid-cols-3 gap-5">
        {testimonials.map((t, i) => (
          ...
          >
            ...
            <blockquote className="text-foreground/85 text-[15px] leading-relaxed">“{t.text}”</blockquote>
        ...
        <div className="mt-10 flex flex-wrap items-center justify-center gap-x-12 gap-y-6">
          {logos.map((l, i) => (
            <motion.div
              ...
              className="font-display text-2xl text-muted-foreground hover:text-foreground transition"
            >
              {l}
            </motion.div>
          ))}
...
function StatItem({ s, start }: { s: (typeof stats)[number]; start: boolean }) {
  ...
  return (
    <div className="text-center">
      ...
      <div className="mt-2 text-muted-foreground text-sm uppercase tracking-widest">{s.label}</div>
    </div>
  );
}
...
function SectionHeader({
  ...
  return (
    <div className="max-w-3xl mx-auto text-center">
      <div className="inline-block text-[11px] uppercase tracking-[0.3em] text-muted-foreground mb-4">
        {kicker}
      </div>
      <h2 className="font-display font-semibold tracking-tight text-foreground text-[clamp(2rem,4.5vw,3.5rem)] leading-[1.05]">
        {title}
      </h2>
      {sub && <p className="mt-4 text-foreground/60 max-w-xl mx-auto">{sub}</p>}
    </div>
  );
}
    ...
    <footer className="relative py-16 px-6 border-t border-border">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-2">
              ...
              <span className="font-display text-foreground">ReasonateAI</span>
          <span className="text-muted-foreground text-xs ml-2">© 2060</span>
        </div>
        <div className="text-muted-foreground text-sm">Built in a Live Twin. Deployed by itself.</div>
      </div>
    </footer>
  );
}
...
// ---------- Root export ----------
export function ModernLanding({ onReplayIntro }: { onReplayIntro?: () => void }) {
    ...
    <div className="relative">
      ...
      {onReplayIntro && (
        <button
          ...
          title="Replay intro"
          className="fixed top-4 right-16 z-50 flex h-9 w-9 items-center justify-center rounded-full border border-border bg-background/80 backdrop-blur transition-colors hover:bg-muted"
        >
